...
public class EnemyPathMoveModule2D : MonoBehaviour
{
    ...
    [SerializeField] private EnemyBase enemy;             // 부모 Enemy 참조
    ...
    private EnemyBase ResolveEnemy()
    {
        // Child에 붙는 전제: 기본은 부모에서 찾고, 없으면 자기 자신에서 찾습니다.
        if (transform.parent != null)
        {
            var parentEnemy = transform.parent.GetComponent<EnemyBase>();
            if (parentEnemy != null) return parentEnemy;
        }

        return GetComponent<EnemyBase>();
    }
    ...
    /// </summary>
    public void Init(EnemyBase enemyRef, Transform[] pathNodes, bool snapToFirstNode = true)
    {
        enemy = enemyRef;
        nodes = pathNodes;
    ...
    public void Init(EnemyBase enemyRef, bool snapToFirstNode = true)
    {
        enemy = enemyRef;
    ...
    /// </summary>
    private float GetBaseSpeedFromEnemy(EnemyBase e)
    {
